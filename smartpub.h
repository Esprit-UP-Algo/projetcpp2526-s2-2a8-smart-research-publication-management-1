...
struct UserAccount {
    QString email;
    QString password;
    QString module;  // "Chercheurs", "Publications", "Finances", "Evenements", "Projets"
    UserRole role;
    QString displayName;
};
...
class IARecommandationsDialog : public QDialog {
    Q_OBJECT
public:
    explicit IARecommandationsDialog(const QList<Projet> &projets, QWidget *parent = nullptr);
...
class StatistiquesDialog : public QDialog {
    Q_OBJECT
public:
    explicit StatistiquesDialog(const QList<Projet> &projets, QWidget *parent = nullptr);
