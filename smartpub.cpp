...
LoginDialog::LoginDialog(QWidget *parent) : QDialog(parent), loggedIn(false) {
...
void LoginDialog::setupAccounts() {
  ...
  accounts << UserAccount{"admin", "admin123", "Tous", UserRole::Admin,
                          "Administrateur"};
...
void LoginDialog::setupUI() {
  ...
  loginTitle->setStyleSheet("font-size: 22px; font-weight: bold; color: "
                            "#1e293b; background: transparent;");
  ...
  emailLabel->setStyleSheet("font-size: 13px; font-weight: 600; color: "
                            "#334155; background: transparent;");
  ...
  passwordLabel->setStyleSheet("font-size: 13px; font-weight: 600; color: "
                               "#334155; background: transparent;");
  ...
  connect(forgotBtn, &QPushButton::clicked, this,
          &LoginDialog::onForgotPasswordClicked);
...
void LoginDialog::onForgotPasswordClicked() {
  QMessageBox::information(this, "Mot de passe oublié",
                           "Veuillez contacter l'administrateur système pour "
...
void FiltresDialog::setupUI() {
  ...
  comboBoxResponsable->addItems({"Tous", "Dr. Ahmed Ben Ali",
                                 "Pr. Fatima Zohra", "Dr. Mohamed Salah",
                                 "Dr. Sarah Johnson", "Pr. Robert Chen"});
  ...
  btnAppliquer->setStyleSheet(
      "QPushButton { background-color: #3b82f6; color: white; border: none; "
      "border-radius: 8px; padding: 10px 20px; }");
  connect(btnAppliquer, &QPushButton::clicked, this, [this]() {
    filtreActif = true;
    accept();
  });
...
QDate FiltresDialog::getDateDebutMax() const { return dateEditMax->date(); }

bool FiltresDialog::isFiltreActif() const { return filtreActif; }

IARecommandationsDialog::IARecommandationsDialog(const QList<Projet> &projets,
                                                 QWidget *parent)
...
void IARecommandationsDialog::setupUI() {
  ...
  for (const auto &rec : m_recommandations) {
    ...
    frame->setStyleSheet(
        "QFrame { background-color: #f8fafc; border: 2px solid #e2e8f0; "
    ...
    QLabel *scoreLabel =
        new QLabel(QString("Score de similarité: %1%")
                       .arg(rec.scoreSimilarite * 100, 0, 'f', 1));
...
void IARecommandationsDialog::genererRecommandations() {
  ...
  if (m_projets.isEmpty())
    return;

  Recommandation rec;
  rec.titre = "Collaboration suggérée";
  rec.description = "Basé sur l'analyse de vos projets, nous suggérons une "
                    "collaboration avec des chercheurs du domaine de l'IA.";
  ...
  rec2.description = "Vos projets pourraient bénéficier d'une meilleure "
...
StatistiquesDialog::StatistiquesDialog(const QList<Projet> &projets,
                                       QWidget *parent)
    : QDialog(parent), m_projets(projets) {
...
void StatistiquesDialog::calculerStatistiques() {
  ...
  for (const auto &p : m_projets) {
    if (p.etat == "Actif")
      actifs++;
    else if (p.etat == "Terminé")
      termines++;
    else if (p.etat == "Planifié")
      planifies++;
    else if (p.etat == "En pause")
      pause++;
  ...
  double progressionMoyenne =
      progressionCount > 0 ? progressionTotal / progressionCount : 0;
  ...
  labelProgressionMoyenne->setText(
      QString("Progression moyenne: %1%").arg(progressionMoyenne, 0, 'f', 1));
...
SmartPub::SmartPub(QWidget *parent)
    : QMainWindow(parent), ui(new Ui::SmartPub), sidebarExpanded(false),
      ...
      cherchIsLoggedIn(false), finVueListeActive(true),
...
void SmartPub::on_cherchBtnRecherche_clicked() {
  ...
  if (cherchVueIconesActive) {
    ...
    if (count == 0) {
      QLabel *noResult = new QLabel("Aucun chercheur trouvé",
                                    ui->cherchScrollAreaWidgetContents);
      noResult->setAlignment(Qt::AlignCenter);
      noResult->setStyleSheet("color: #94a3b8; font-size: 16px; margin-top: "
                              "50px; background: transparent; border: none;");
...
void SmartPub::cherchTrierParGrade() {
  QMap<QString, int> gradeOrder = {
      {"Professeur", 1},     {"Maitre de Conferences", 2},
      {"Docteur", 3},        {"Ingenieur de Recherche", 4},
      {"Post-doctorant", 5}, {"Doctorant", 6}};
...
void SmartPub::on_cherchModifierChercheur(int id) {
  ...
  editNom->setStyleSheet("padding: 12px; border-radius: 10px; border: 2px "
                         "solid #e2e8f0; font-size: 14px;");
  QLineEdit *editPrenom = new QLineEdit(data.prenom);
  editPrenom->setStyleSheet("padding: 12px; border-radius: 10px; border: 2px "
                            "solid #e2e8f0; font-size: 14px;");
  QLineEdit *editEmail = new QLineEdit(data.email);
  editEmail->setStyleSheet("padding: 12px; border-radius: 10px; border: 2px "
                           "solid #e2e8f0; font-size: 14px;");
...
void SmartPub::projSetupUI() {
  ...
  if (ui->btnListeProjets)
    ui->btnListeProjets->setStyleSheet(toolbarBtnStyle);
  if (ui->btnAjouterProjet)
    ui->btnAjouterProjet->setStyleSheet(toolbarBtnStyle);
  if (ui->btnModifierProjet)
    ui->btnModifierProjet->setStyleSheet(toolbarBtnStyle);
  if (ui->btnSupprimerProjet)
    ui->btnSupprimerProjet->setStyleSheet(toolbarBtnStyle);
  if (ui->btnStatistiques)
    ui->btnStatistiques->setStyleSheet(toolbarBtnStyle);
  if (ui->btnFiltresProjets)
    ui->btnFiltresProjets->setStyleSheet(toolbarBtnStyle);
  if (ui->btnExporterProjets)
    ui->btnExporterProjets->setStyleSheet(toolbarBtnStyle);

  // Boutons de tri (checkable)
  if (ui->btnTriDateDebut)
    ui->btnTriDateDebut->setStyleSheet(checkableBtnStyle);
  if (ui->btnTriDateFin)
    ui->btnTriDateFin->setStyleSheet(checkableBtnStyle);
  if (ui->btnTriEtat)
    ui->btnTriEtat->setStyleSheet(checkableBtnStyle);
  if (ui->btnTriProgression)
    ui->btnTriProgression->setStyleSheet(checkableBtnStyle);
...
void SmartPub::projConnectSignals() {
  connect(ui->btnListeProjets, &QPushButton::clicked, this,
          &SmartPub::on_btnListeProjets_clicked);
  connect(ui->btnAjouterProjet, &QPushButton::clicked, this,
          &SmartPub::on_btnAjouterProjet_clicked);
  connect(ui->btnModifierProjet, &QPushButton::clicked, this,
          &SmartPub::on_btnModifierProjet_clicked);
  connect(ui->btnSupprimerProjet, &QPushButton::clicked, this,
          &SmartPub::on_btnSupprimerProjet_clicked);

  connect(ui->lineEditRechercheProjets, &QLineEdit::textChanged, this,
          &SmartPub::on_lineEditRechercheProjets_textChanged);
  connect(ui->btnAnnulerForm, &QPushButton::clicked, this,
          &SmartPub::on_btnAnnulerForm_clicked);
  connect(ui->btnEnregistrerForm, &QPushButton::clicked, this,
          &SmartPub::on_btnEnregistrerForm_clicked);
  ...
  connect(ui->btnTriDateDebut, &QPushButton::clicked, this,
          &SmartPub::on_triDateDebutClicked);
  connect(ui->btnTriDateFin, &QPushButton::clicked, this,
          &SmartPub::on_triDateFinClicked);
  connect(ui->btnTriEtat, &QPushButton::clicked, this,
          &SmartPub::on_triEtatClicked);
  connect(ui->btnTriProgression, &QPushButton::clicked, this,
          &SmartPub::on_triProgressionClicked);

  connect(ui->btnStatistiques, &QPushButton::clicked, this,
          &SmartPub::on_statistiquesClicked);
  connect(ui->btnFiltresProjets, &QPushButton::clicked, this,
          &SmartPub::on_filtresClicked);
  connect(ui->btnExporterProjets, &QPushButton::clicked, this,
          &SmartPub::on_exporterClicked);
}
...
void SmartPub::projAjouterProjetTable(const Projet &projet, int rowIndex) {
  ...
  // Date début
  QTableWidgetItem *debutItem =
      new QTableWidgetItem(projet.dateDebut.toString("dd/MM/yyyy"));
  ...
  // Date fin
  QTableWidgetItem *finItem =
      new QTableWidgetItem(projet.dateFin.toString("dd/MM/yyyy"));
  ...
  progItem->setForeground(
      QColor(getProgressionColorFromString(projet.progression)));
...
void SmartPub::projAjusterColonnesTable() {
  ...
  if (table->columnWidth(1) < 100)
    table->setColumnWidth(1, 100);
  if (table->columnWidth(2) < 250)
    table->setColumnWidth(2, 250);
  if (table->columnWidth(2) > 400)
    table->setColumnWidth(2, 400);
  if (table->columnWidth(3) < 90)
    table->setColumnWidth(3, 90);
  if (table->columnWidth(4) < 90)
    table->setColumnWidth(4, 90);
  if (table->columnWidth(5) < 150)
    table->setColumnWidth(5, 150);
  if (table->columnWidth(6) < 80)
    table->setColumnWidth(6, 80);
  if (table->columnWidth(7) < 80)
    table->setColumnWidth(7, 80);
}

void SmartPub::projViderTable() { ui->tableWidgetProjets->setRowCount(0); }

void SmartPub::projSetActiveCrudButton(int index) {
  QVector<QPushButton *> buttons = {ui->btnListeProjets, ui->btnAjouterProjet,
                                    ui->btnModifierProjet,
                                    ui->btnSupprimerProjet};
...
void SmartPub::on_btnAjouterProjet_clicked() {
  if (currentUser.role == UserRole::Guest) {
    QMessageBox::warning(this, "Accès refusé",
                         "Les invités ne peuvent pas ajouter de projets.");
    return;
  }
...
void SmartPub::on_btnModifierProjet_clicked() {
  if (currentUser.role == UserRole::Guest) {
    QMessageBox::warning(this, "Accès refusé",
                         "Les invités ne peuvent pas modifier de projets.");
    return;
  }
...
void SmartPub::on_btnSupprimerProjet_clicked() {
  if (currentUser.role == UserRole::Guest) {
    QMessageBox::warning(this, "Accès refusé",
                         "Les invités ne peuvent pas supprimer de projets.");
    return;
  }
...
void SmartPub::projMettreAJourProjetTable(int row, const Projet &projet) {
  ...
  ui->tableWidgetProjets->item(row, 3)->setText(
      projet.dateDebut.toString("dd/MM/yyyy"));
  ui->tableWidgetProjets->item(row, 4)->setText(
      projet.dateFin.toString("dd/MM/yyyy"));
...
void SmartPub::on_triDateDebutClicked() {
  ...
  btn->setChecked(true);
  btn->setText(currentSortOrder == Qt::AscendingOrder ? "📅 Date Début ▲"
                                                      : "📅 Date Début ▼");
}
...
void SmartPub::on_triDateFinClicked() {
  ...
  projSortProjetsBy(currentSortColumn, currentSortOrder);
  btn->setChecked(true);
  btn->setText(currentSortOrder == Qt::AscendingOrder ? "📅 Date Fin ▲"
                                                      : "📅 Date Fin ▼");
}
...
void SmartPub::on_triEtatClicked() {
  ...
  projSortProjetsBy(currentSortColumn, currentSortOrder);
  btn->setChecked(true);
  btn->setText(currentSortOrder == Qt::AscendingOrder ? "🔧 État ▲"
                                                      : "🔧 État ▼");
}
...
void SmartPub::on_triProgressionClicked() {
  ...
  projSortProjetsBy(currentSortColumn, currentSortOrder);
  btn->setChecked(true);
  btn->setText(currentSortOrder == Qt::AscendingOrder ? "📈 Progression ▲"
                                                      : "📈 Progression ▼");
}
...
void SmartPub::projSortProjetsBy(int column, Qt::SortOrder order) {
  ...
  case 3:
    std::sort(projets.begin(), projets.end(),
              [order](const Projet &a, const Projet &b) {
                return order == Qt::AscendingOrder ? a.dateDebut < b.dateDebut
  ...
  case 4:
    std::sort(projets.begin(), projets.end(),
              [order](const Projet &a, const Projet &b) {
                return order == Qt::AscendingOrder ? a.dateFin < b.dateFin
                                                   : a.dateFin > b.dateFin;
              });
    break;
  case 6:
    std::sort(projets.begin(), projets.end(),
              [order](const Projet &a, const Projet &b) {
                return order == Qt::AscendingOrder ? a.etat < b.etat
                                                   : a.etat > b.etat;
              });
    break;
  case 7:
    std::sort(
        projets.begin(), projets.end(),
        [order](const Projet &a, const Projet &b) {
...
void SmartPub::on_santeProjetClicked() {
  ...
  message +=
      QString("<span style='color: #10b981;'>Sains: %1</span><br>").arg(sains);
  message += QString("<span style='color: #f59e0b;'>À risque: %1</span><br>")
                 .arg(risque);
  message += QString("<span style='color: #ef4444;'>Critiques: %1</span><br>")
                 .arg(critiques);
...
void SmartPub::on_optimiserChargeClicked() {
  QMessageBox::information(this, "Optimisation de Charge",
                           ...
                           "Cette fonctionnalité analysera la répartition des "
...
void SmartPub::on_iaRecommanderClicked() {
  QList<Projet> projetsList(projets.begin(), projets.end());
  IARecommandationsDialog *dialog =
      new IARecommandationsDialog(projetsList, this);
  dialog->setAttribute(Qt::WA_DeleteOnClose);
  dialog->exec();
}
...
void SmartPub::projAppliquerFiltres() {
  ...
  for (const auto &projet : projets) {
    ...
    if (!filtreResponsable.isEmpty() &&
        projet.responsable != filtreResponsable) {
      match = false;
    }

    if (projet.dateDebut < filtreDateDebutMin ||
        projet.dateDebut > filtreDateDebutMax) {
      match = false;
    }
...
void SmartPub::projMettreAJourBadgeFiltres() {
  if (filtresActifs) {
    ...
    ui->btnFiltresProjets->setStyleSheet(
        ...
        "QPushButton:hover {"
        "    background: qlineargradient(x1:0, y1:0, x2:1, y2:0, stop:0 "
        "#2563eb, stop:1 #059669);"
        "}");
  } else {
    ui->btnFiltresProjets->setText("🎛️ Filtres");
    ui->btnFiltresProjets->setStyleSheet("QPushButton {"
                                         "    background-color: white;"
                                         "    color: #334155;"
                                         "    border: 1px solid #e2e8f0;"
                                         "    border-radius: 10px;"
...
void SmartPub::on_exporterClicked() {
  QString fileName = QFileDialog::getSaveFileName(this, "Exporter les projets",
                                                  "", "Fichiers CSV (*.csv)");
  if (!fileName.isEmpty()) {
    QMessageBox::information(this, "Exportation",
                             "Projets exportés avec succès dans:\n" + fileName);
  }
}
...
bool SmartPub::projValiderFormulaire() const {
  if (ui->lineEditCodeForm->text().isEmpty()) {
    QMessageBox::warning(const_cast<SmartPub *>(this), "Validation",
                         "Le code du projet est requis");
  ...
  if (ui->dateEditDebutForm->date() > ui->dateEditFinForm->date()) {
    QMessageBox::warning(
        const_cast<SmartPub *>(this), "Validation",
...
double SmartPub::projCalculerTauxAvancement(const Projet &projet) const {
  ...
  double pourcentageTemps =
      (1.0 - (double)joursRestants / (double)joursTotaux) * 100;
...
void SmartPub::SR_addButtonsToRow(int row) {
  ...
  pLayout->setContentsMargins(0, 0, 0, 0); // Reduced margins
  pLayout->setSpacing(5);

  QPushButton *btnEdit = new QPushButton("Modifier");
  btnEdit->setStyleSheet(
      "QPushButton { background-color: #3b82f6; color: white; border-radius: "
      "4px; padding: 4px; border: none; min-width: 60px; min-height: 24px; } "
      "QPushButton:hover { "
      "background-color: #2563eb; }");
  ...
  // Ensure the widget itself is visible and has a logical size hint
  pWidget->show();
  ...
  connect(btnEdit, &QPushButton::clicked, this, [this]() {
    if (currentUser.role == UserRole::Guest) {
      QMessageBox::warning(
          this, "Accès refusé",
          "Les invités ne peuvent pas modifier de publications.");
      return;
    }
