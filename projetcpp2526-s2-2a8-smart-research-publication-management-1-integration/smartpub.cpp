...
SmartPub::SmartPub(QWidget *parent)
    ...
    if (arduino->connect_arduino() == 0) {
        ...
        // Scénario 1 : accès labo (RFID). Scénario 3 : clôture projet (CLOTURER:)
        scenarioAcces = new Scenario1(arduino, 1);
        scenarioCloture = new Scenario3(arduino);
        ...
        connect(arduino->getserial(), &QSerialPort::readyRead, this, [this]() {
            ...
            if (message.startsWith("CLOTURER:", Qt::CaseInsensitive))
                scenarioCloture->processClotureFromMessage(message);
            else
                scenarioAcces->processAccessFromMessage(message);
        });
    } else {
