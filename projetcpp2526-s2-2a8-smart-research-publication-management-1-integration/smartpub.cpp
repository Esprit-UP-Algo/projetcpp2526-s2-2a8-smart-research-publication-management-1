...
SmartPub::SmartPub(QWidget *parent)
    ...
    arduino(nullptr), scenarioAcces(nullptr), scenarioCloture(nullptr),
    scenarioIdentification(nullptr), labelRfidStatus(nullptr) {
    ui->setupUi(this);
    ...
    // =====================================================================
    // ARDUINO — scénario 1 (accès labo), 2 (clôture projet), 3 (RFID chercheurs)
    // Une seule ligne série lue ici, puis dispatch selon le préfixe / module actif
    // =====================================================================
    arduino = new Arduino();
    if (arduino->connect_arduino() == 0) {
        qDebug() << "[SmartPub] Arduino connecté sur"
        ...
        connect(arduino->getserial(), &QSerialPort::readyRead, this, [this]() {
            const QString ligne = arduino->readLine();
            if (ligne.isEmpty())
                return;
            ...
            if (message.startsWith(QStringLiteral("CLOTURER:"),
                                    Qt::CaseInsensitive)) {
                scenarioCloture->processCloture(message);
                return;
            }
            ...
            if (message.startsWith(QStringLiteral("RFID:"), Qt::CaseInsensitive)) {
                ...
                if (moduleActif == 0) {
                    scenarioIdentification->processIdentification(message);

                    if (labelRfidStatus) {
                        if (scenarioIdentification->dernierResultat()) {
                            labelRfidStatus->setText(
                                QStringLiteral("Chercheur trouvé : %1")
                                    .arg(scenarioIdentification->dernierNom()));
                            ...
                            labelRfidStatus->setText(
                                QStringLiteral("Accès bloqué — CIN inconnu : %1")
                                    .arg(scenarioIdentification->dernierCin()));
                            labelRfidStatus->setStyleSheet(
                                QStringLiteral(
                                    "QLabel { color: white; font-size: 13px;"
                                    " font-weight: 700; padding: 8px 16px;"
                        ...
                        QTimer::singleShot(4000, this, [this]() {
                            if (!labelRfidStatus)
                                return;
                            labelRfidStatus->setText(
                                QStringLiteral("En attente de carte RFID…"));
                            labelRfidStatus->setStyleSheet(
                                QStringLiteral(
                    ...
                    scenarioAcces->processAccess(message);
                }
                return;
            }

            qDebug() << "[SmartPub] Message série non routé :" << message;
        });
    } else {
        qDebug() << "[SmartPub] Échec de connexion à l'Arduino";
    }
