...
void SmartPub::cherchSetupUI() {
    ...
    // ── Statut RFID (scénario 3) + ouverture du journal ──────────────────────
    if (!labelRfidStatus && ui->horizontalLayoutToolbar) {
        labelRfidStatus = new QLabel(tr("En attente de carte RFID…"), this);
        labelRfidStatus->setAlignment(Qt::AlignCenter);
        labelRfidStatus->setMinimumHeight(36);
        ...
        auto *btnJournalRfid = new QPushButton(tr("Journal RFID"), this);
        btnJournalRfid->setCursor(Qt::PointingHandCursor);
        btnJournalRfid->setStyleSheet(
            QStringLiteral(
        ...
        connect(btnJournalRfid, &QPushButton::clicked, this, [this]() {
            RfidJournal::afficherJournal(this);
        });

        auto *rfidWrap = new QWidget(this);
        auto *h = new QHBoxLayout(rfidWrap);
        h->setContentsMargins(0, 0, 0, 0);
        h->setSpacing(8);
        h->addWidget(labelRfidStatus, 1);
        h->addWidget(btnJournalRfid, 0);
        ui->horizontalLayoutToolbar->addWidget(rfidWrap);
    }
